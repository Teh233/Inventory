...
const SellerProfile = () => {
  ...
  const { sellerId, name, email, createdAt } = useSelector(
    (state) => state?.auth?.userInfo
  );
  const {data:personalDetails} = useGetSellerDetailsQuery(sellerId)
  const companyLogo= (personalDetails?.details?.companyLogo)
  ...
  const handleChangePassword = () => {
    navigate(`/changePassword/${sellerId}`);
  };
  ...
  return (
    ...
    >
      ...
      <Box
        ...
        sx={{ maxWidth: "500px" }}
      >
        <Avatar variant="square" sx={{ width: 100, height: 100, mb: 2 }}  src={personalDetails?.details?.companyLogo?.url}/>
